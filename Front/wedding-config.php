...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $per_head = isset($_POST['per_head']) ? (int)$_POST['per_head'] : 0;
    ...
    // Upload each set of requirements
    for ($i = 0; $i < 5; $i++) {
        $key_name = "requirements" . ($i ? $i : "");
        if (isset($_FILES[$key_name])) {
            foreach ($_FILES[$key_name]['name'] as $key => $name) {
                $file_type = $_FILES[$key_name]['type'][$key];
                $target_file = $target_dir . basename($name);

                if (in_array($file_type, $allowed_file_types)) {
                    if (move_uploaded_file($_FILES[$key_name]['tmp_name'][$key], $target_file)) {
                        $requirements_files[$i][] = $name;
                    } else {
                        echo "Error uploading the file: " . htmlspecialchars($name);
                    }
                } else {
    ...
    $stmt = $conn->prepare(
        "INSERT INTO reservation (
            uid, service_type, s_description, s_description1, set_date, 
            time_slot, valid_id, s_requirements, s_requirements1, s_requirements2, 
            s_requirements3, s_requirements4, per_head, r_type, amount, payment_type
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    );

    // Convert file names to JSON string for storing in database
    $valid_id_json = json_encode($valid_id_files);
    $requirements_json = array_map(fn($files) => json_encode($files), $requirements_files);
    ...
    if ($stmt->execute()) {
        ...
        header("Location: wedding-preview.php");
        exit();
    } else {
