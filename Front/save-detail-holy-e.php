...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmtUpdate = $conn->prepare("
        UPDATE reservation 
        SET service_type = ?, s_description = ?, s_description1 = ?,  set_date = ?, time_slot = ?, s_address = ?, amount = ?, payment_type = ?, valid_id = ?, updated_at = NOW() 
        WHERE s_id = ?
    ");
    ...
    if ($stmtUpdate->execute()) {
        header("Location: holy-eucharist-preview.php?s_id=" . $s_id);
        exit();
    } else {
