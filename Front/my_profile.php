...
if (mysqli_stmt_prepare($stmt, $sql)) {
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        ...
        // Redirect to user-profile-setup.php if profile is not completed
        if ($row['profile_completed'] == '0') {
            header("Location: user-profile-setup.php");
            exit();
        }
...
$user_id = $_SESSION['uid'];  // Use the logged-in user's ID from the session
$sqlUserDetails = "SELECT userimg, firstname, lastname, username, contactnum, email, address,Q1,A1, gender FROM users WHERE uid = ?";
...
// Ensure profile image has a fallback if not set
$profileImage = isset($UserDetails['userimg']) && !empty($UserDetails['userimg']) ? $UserDetails['userimg'] : 'css/img/default-profile.png';

// Set default values for missing keys to avoid undefined array warnings
...
$contactnum = isset($UserDetails['contactnum']) ? $UserDetails['contactnum'] : '';
...
$Q1 = isset($UserDetails['Q1']) ? $UserDetails['Q1'] : '';
$A1 = isset($UserDetails['A1']) ? $UserDetails['A1'] : '';
?>
...
.a-profile-div {
    animation: fadeUp 0.5s ease-in-out; /* Apply fade-up animation */
}
...
.overlay-updated .overlay-content {
    padding: 20px;
    border-radius: 10px;
    text-align: center;
    color:#fff;
...
<div class="a-profile-div">
    ...
    <form action="profile_config.php" method="POST" enctype="multipart/form-data">
        ...
        <div class="cnt-input-a">
            ...
            <input type="text" name="username" id="username" value="<?php echo htmlspecialchars($username); ?>" style="background:rgba(0,0,0,.1); color:#fff; cursor:default;" readonly>
        </div>
        ...
        <div class="cnt-input-a">
            ...
            <input type="email" name="email" id="email" value="<?php echo htmlspecialchars($email); ?>" style="background:rgba(0,0,0,.1); color:#fff; cursor:default;" readonly>
        </div>
        ...
        <div class="cnt-input-a">
            ...
            <select name="address" id="address" required>
                ...
                <option value="Brgy. Agujo" <?php echo ($address == 'Brgy. Agujo') ? 'selected' : ''; ?>>Brgy. Agujo</option>
                <option value="Brgy. Bakhawan" <?php echo ($address == 'Brgy. Bakhawan') ? 'selected' : ''; ?>>Brgy. Bakhawan</option>
                <option value="Brgy. Bateria" <?php echo ($address == 'Brgy. Bateria') ? 'selected' : ''; ?>>Brgy. Bateria</option>
                <option value="Brgy. Bitoon" <?php echo ($address == 'Brgy. Bitoon') ? 'selected' : ''; ?>>Brgy. Bitoon</option>
                <option value="Brgy. Bagay" <?php echo ($address == 'Brgy. Bagay') ? 'selected' : ''; ?>>Brgy. Bagay</option>
                <option value="Brgy. Calape" <?php echo ($address == 'Brgy. Calape') ? 'selected' : ''; ?>>Brgy. Calape</option>
                <option value="Brgy. Dalingding" <?php echo ($address == 'Brgy. Dalingding') ? 'selected' : ''; ?>>Brgy. Dalingding</option>
                <option value="Brgy. Lanao" <?php echo ($address == 'Brgy. Lanao') ? 'selected' : ''; ?>>Brgy. Lanao</option>
                <option value="Brgy. Maya" <?php echo ($address == 'Brgy. Maya') ? 'selected' : ''; ?>>Brgy. Maya</option>
                <option value="Brgy. Malbago" <?php echo ($address == 'Brgy. Malbago') ? 'selected' : ''; ?>>Brgy. Malbago</option>
                <option value="Brgy. Malingin" <?php echo ($address == 'Brgy. Malingin') ? 'selected' : ''; ?>>Brgy. Malingin</option>
                <option value="Brgy. Poblacion" <?php echo ($address == 'Brgy. Poblacion') ? 'selected' : ''; ?>>Brgy. Poblacion</option>
                <option value="Brgy. Paypay" <?php echo ($address == 'Brgy. Paypay') ? 'selected' : ''; ?>>Brgy. Paypay</option>
                <option value="Brgy. Pajo" <?php echo ($address == 'Brgy. Pajo') ? 'selected' : ''; ?>>Brgy. Pajo</option>
                <option value="Brgy. Tominjao" <?php echo ($address == 'Brgy. Tominjao') ? 'selected' : ''; ?>>Brgy. Tominjao</option>
                <option value="Brgy. Tapilon" <?php echo ($address == 'Brgy. Tapilon') ? 'selected' : ''; ?>>Brgy. Tapilon</option>
                <option value="Brgy. Talisay" <?php echo ($address == 'Brgy. Talisay') ? 'selected' : ''; ?>>Brgy. Talisay</option>
                <option value="Brgy. Tinubdan" <?php echo ($address == 'Brgy. Tinubdan') ? 'selected' : ''; ?>>Brgy. Tinubdan</option>
                <option value="Other" <?php echo ($address == 'Other') ? 'selected' : ''; ?>>Other</option>
            </select>
        </div>
        
        <div class="cnt-input-a">
            <label for="Q1">Security Question: <small style="font-weight:300; font-size:14px;"> <?php echo htmlspecialchars($Q1); ?> </small></label>
            <input 
        type="password" 
        id="email" 
        value="<?php echo htmlspecialchars($A1); ?>" 
        style="background:rgba(0,0,0,.1); color:#fff; cursor:default;" 
        readonly
  ...
  <!-- Overlay for update success message -->
  <div class="overlay-updated" id="overlay-updated" style="display: none;">
    ...
    function showOverlay() {
        const overlay = document.getElementById('overlay-updated');
        overlay.style.display = 'flex';
        overlay.style.opacity = '0';
        setTimeout(() => {
            overlay.style.opacity = '1';
        }, 10); // Small timeout for smooth fade-in
        setTimeout(() => {
