...
<a href="edit_reservation_baptism.php?s_id=<?php echo $reservation['s_id']; ?>">Edit Reservation</a>
<button type="submit" name="delete_reservation" id="button-delete">Cancel Reservation</button>
</form>

<script>
    // Show the loading screen when "Submit" is clicked
    document.getElementById('nextButton').addEventListener('click', function (event) {
        ...
        // Simulate delay for visual confirmation of the loading screen (remove in production)
        setTimeout(() => {
            // Redirect based on the payment type
            if (paymentType === 'Gcash (Scan / Send Money)') {
                window.location.href = 'gateway1-payment.php?s_id=' + s_id;
            } else if (paymentType === 'Over The Counter') {
                window.location.href = 'gateway2-payment.php?s_id=' + s_id;
            } else {
