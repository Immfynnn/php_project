...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmtUpdate = $conn->prepare("UPDATE reservation SET service_type = ?, s_description = ?, set_date = ?, r_type = ?, time_slot = ?, s_address = ?, valid_id = ?, updated_at = NOW() WHERE s_id = ?");
    $stmtUpdate->bind_param("sssssssi", $service_type, $s_description, $set_date, $r_type, $time_slot, $s_address,$valid_id_path, $s_id);

    if ($stmtUpdate->execute()) {
        header("Location: blessing-preview2.php?s_id=" . $s_id);
        exit();
    } else {
