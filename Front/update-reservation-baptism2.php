...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_reservation'])) {
    ...
    $r_type  = $_POST['r_type'];
    $per_head = isset($_POST['per_head']) ? $_POST['per_head'] : 0;
...
$stmtUpdate = $conn->prepare("UPDATE reservation SET service_type = ?, s_description = ?, set_date = ?,r_type =?,  per_head =?, time_slot = ?, s_address = ?, amount = ?, payment_type = ?, valid_id = ?, s_requirements = ?, updated_at = NOW() WHERE s_id = ?");
$stmtUpdate->bind_param("ssssissssssi", $service_type, $s_description, $schedule,$r_type, $per_head,  $time_slot, $s_address, $amount, $payment_type, $valid_id_path, $requirements_path, $s_id);

if ($stmtUpdate->execute()) {
    // Redirect after successful update
    header("Location: preview-details-baptism.php?s_id=" . $s_id);
    exit();
} else {
