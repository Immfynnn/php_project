...
if ($result->num_rows > 0) {
    ...
    ?>
   <form action="otc-config.php" method="POST">
    <div class="otc-cont" id="animatedDiv">
        <div class="header">
            <h3>Over The Counter</h3>
        </div>
        <div class="reservation-details">
            <p> Present the receipt to the person-in-charge in the church to process your payment. </p><br> 
            <p>If you have any questions regarding the services or need further assistance, feel free to contact our support team.</p>
        </div>
        <div class="reservation-details">
            <p>Service Name: <strong><?php echo htmlspecialchars($reservation['service_type']); ?></strong></p>
            <input type="text" name="total_amount" value="<?php echo htmlspecialchars($reservation['amount']); ?>" style="padding:15px; outline:solid 1px rgba(0,0,0,.2); background:rgba(0,0,0,.1); cursor:default; width:100%; border-radius:5px;" readonly>
        </div>
        
        <button class="btn-submit-row">Proceed</button>
    </div>
    </form>
        </div>
        </main>
        ...
        <script>
        // JavaScript to add the slide-up animation class
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (animatedDiv) {
                ...
                console.error("Animated div not found");
            }
        });
    </script>
