...
// Check if message_id is passed
if (!isset($_GET['message_id'])) {
    echo "<p style='color:red;'>No message selected. Please go back to the messages page.</p>";
    exit();
}
...
$sql = "SELECT m.message_id, 
               ...
               m.message_content, 
               m.sent_at,
               m.image_upload -- Fetch image upload field from the database
        FROM messages m
...
if ($stmt->execute()) {
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<p style='color:red;'>Message not found or you are not authorized to view this message.</p>";
        exit();
    }
} else {
...
<main>
    <div class="cont-inbox fade-up-animation" style="background:#1A6B96; color:#fff; width:100%; display:flex; justify-content:center; align-items:center; flex-direction:column; padding:30px; border-radius:10px; box-shadow:0 5px 10px rgba(0,0,0,.5);">
        <form id="delete-all-form" action="delete_all_messages.php" method="POST"> 
            <h2 style="text-align:left;">Message Details</h2>
        </form>
        <div class="div-head-msg">
            <!-- Message Content -->
            <div class="message-content" style="background:rgba(255,255,255,.1); color:#fff; padding:20px; border-radius:10px;">
                <p><strong>From:</strong> <?php echo htmlspecialchars($message['sender_username']); ?></p>
                <p><strong>Sent At:</strong> <?php echo htmlspecialchars(date('F d, Y, g:i A', strtotime($message['sent_at']))); ?></p><br>
                <p><strong>Message:</strong></p><br>
                <p><?php echo nl2br(htmlspecialchars($message['message_content'])); ?></p>

                <!-- Display Image Upload if it exists -->
                <?php if (!empty($message['image_upload']) && $message['image_upload'] !== 'black'): ?>
                    <img src="<?php echo htmlspecialchars($message['image_upload']); ?>" alt="Uploaded Image" style="max-width: 90%; height: auto; border-radius: 10px; margin-top: 10px;">
                <?php endif; ?>

                <br>
                <br>
                <button id="replyButton" style="padding:10px; padding-left:30px; padding-right:30px; cursor:pointer; border-radius:5px;border:none; outline:none;">Reply</button>
            </div>
        </div>
    </div>

    <!-- Reply Overlay (Hidden initially) -->
    <div id="replyOverlay" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); z-index:9999; justify-content:center; align-items:center; flex-direction:column;">
        <!-- Form Container to slide down -->
        <div id="replyForm" style="display:none; background:white; padding:20px; border-radius:10px; width:80%; max-width:600px; animation: slideDown 0.5s ease;">
        <form action="reply-admin.php" method="POST" enctype="multipart/form-data">
            <h4 style="margin-bottom:20px;">Reply to Message
            <input type="text" value="<?php echo htmlspecialchars($message['sender_username']); ?>" name="recipient_admin_id_or_username" style=" font-size:18px;outline:none; border:none; font-weight:800;" readonly></h4> 
            <input type="file" name="image_upload"  name="image_upload"  style="margin-bottom:10px;">
            <textarea id="replyMessage" style="width:100%; height:150px; padding:10px; border-radius:5px;" name="message_cont" placeholder="Type your reply here..."></textarea>
            <div style="margin-top:10px; text-align:right;">
                <button id="sendReplyButton" style="padding:10px 20px; background:#1A6B96; color:#fff; border:none; border-radius:5px; cursor:pointer;">Send Reply</button>
                <button id="closeReplyOverlay" type="button" style="padding:10px 20px; background:#ccc; color:#333; border:none; border-radius:5px; cursor:pointer; margin-left:10px;">Cancel</button>
            </div>
                </form>
        </div>
    </div>
    ...
    <div id="successMessageContainer" style="background:white; padding:20px; border-radius:10px; text-align:center;">
        <h2 id="successMessage" style="margin:0; color:green;">Message Sent Successfully!</h2>
    </div>
</div>
...
@keyframes slideUp {
    ...
    to {
        transform: translateY(-100%);  /* End above the screen */
    }
}
</style>

<script>
// JavaScript to show overlay with slide-down animation when clicking "Reply" button
document.getElementById('replyButton').addEventListener('click', function() {
    document.getElementById('replyOverlay').style.display = 'flex'; // Show overlay
    document.getElementById('replyForm').style.display = 'block'; // Show form with slide down effect
    document.getElementById('replyForm').style.animation = 'slideDown 0.5s ease'; // Apply slide down animation
});

// JavaScript to close the overlay when clicking "Cancel"
document.getElementById('closeReplyOverlay').addEventListener('click', function() {
    document.getElementById('replyForm').style.animation = 'slideUp 0.5s ease'; // Slide up animation
    setTimeout(function() {
        document.getElementById('replyOverlay').style.display = 'none';
        document.getElementById('replyForm').style.display = 'none';
    }, 500); // Matches the duration of the slide-up animation
});

// JavaScript to handle form submission and show success overlay
document.querySelector('#replyForm form').addEventListener('submit', function(event) {
    event.preventDefault(); // Prevent the default form submission

    const formData = new FormData(this); // Gather form data

    fetch('reply-admin.php', {
        method: 'POST',
        body: formData
    })
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            // Apply the fade-out animation after 2 seconds and redirect after fade-out is complete
            setTimeout(() => {
