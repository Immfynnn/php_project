...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function (event) {
        ...
        <form action="save-detail-blessing.php" method="POST" enctype="multipart/form-data">
        <div class="input-b">
    <input type="hidden" id="service_type" name="service_type" value="<?php echo htmlspecialchars($reservation['service_type']); ?>" required>
</div>

        <div class="input-b">
        <label for="s_description">Full Name:</label>
        <input type="text" id="s_description" name="s_description" value="<?php echo htmlspecialchars($reservation['s_description']); ?>" required>
    </div>  
    <div class="input-b">
        <label for="r_type">Type of Blessing:</label>
        <input type="text" id="r_type" name="r_type" value="<?php echo htmlspecialchars($reservation['r_type']); ?>" required>
    </div>  
    <div class="input-b">
        <label for="set_date">Schedule:</label>
        <input type="date" id="set_date" name="set_date" value="<?php echo htmlspecialchars($reservation['set_date']); ?>" required>
    </div>
    ...
    <select name="time_slot" id="time-slot">
        ...
        $time_slots = [
          ...
          $time_slot_from_db = null;
          if (isset($s_id)) {
              $stmt = $conn->prepare("SELECT time_slot, s_address, amount, valid_id FROM reservation WHERE s_id = ?");
              $stmt->bind_param("i", $s_id);
              $stmt->execute();
              $result = $stmt->get_result();
              if ($result->num_rows > 0) {
                  $reservation = $result->fetch_assoc();
                  $time_slot_from_db = $reservation['time_slot']; // Get the time_slot from the database
              }
          }

        // Loop through the time slots and create <option> elements
        foreach ($time_slots as $slot) {
            // Check if the slot matches the database value
            $selected = ($time_slot_from_db === $slot) ? 'selected' : '';
...
<div class="input-b">
       ...
       <input type="text" name="s_address" id="s_address"  value="<?php echo htmlspecialchars($reservation["s_address"]); ?>"required>
    </div>
...
<div class="input-b">
    ...
    <input type="file" id="validId" name="validId[]" accept="image/*,application/pdf,.docx,.doc" multiple />
    <?php if (!empty($reservation['valid_id'])): ?>
        <div>
            <label>Current Valid ID(s):</label>
            <?php 
                $valid_ids = json_decode($reservation['valid_id'], true);
                foreach ($valid_ids as $file): 
            ?>
                <p><a href="uploads/<?php echo htmlspecialchars($file); ?>" target="_blank" style="color:blue;"><?php echo htmlspecialchars($file); ?></a></p>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
</div>
    ...
    <input type="number" id="ammount" name="amount" value="<?php echo htmlspecialchars($reservation["amount"]); ?>" placeholder="Any Amount" oninput="showPaymentType()" style="width:100%;">
</div>
...
<div id="payment_type_container"></div>
    ...
    <div class="container py-5" id="page-container">
        ...
        <div class="content-detail-serv1" style="background:url('css/img/Blessing_HolyWater.jpg'); background-size:cover; background-position:center;">
        <div class="bg-content-bapt">  
          <h1 style="font-size:40px; text-transform:uppercase; font-weight:800;">Blessing</h1>
          A blessing is a ceremonial or ritual act that invokes divine favor, protection, or guidance upon individuals, objects, places, or events. Found across various cultures and religions, blessings are expressions of goodwill and spiritual significance, often performed by religious leaders, elders, or individuals recognized for their spiritual authority.
        </div>
    </div>
