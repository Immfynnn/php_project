...
border-radius:5px; color:#fff;">Back</a>
        ...
        <form action="baptism-config.php" method="POST" enctype="multipart/form-data">
            <div class="input-b">
            ...
            <div class="input-b">
                ...
                <input type="text" id="description" name="description" value="<?php echo isset($description) ? htmlspecialchars($description) : ''; ?>" required placeholder="Christian Name and other details">
            </div>

            <div class="input-b">
                <label for="schedule" style="color:#fff;">Schedule:</label>
            ...
            <p style="font-size:15px; font-weight:bold; margin-bottom:10px;" >Select the 2 option 'Special' or 'Regular'</p>
        <div class="radio-cnt-1" id="regularContainer" style="background:skyblue; transition: opacity 0.3s;">
            <label class="header-radio">
                <h3>Regular</h3>
                <div class="right">
                    <h5>PHP200.00</h5>
                    <input type="radio" name="r_type" id="regularRadio" class="regular" value="Regular" required>
                </div>
            </label>
        </div>
        <div class="radio-cnt-1" id="specialContainer" style="background:#90EE90; transition: opacity 0.3s;">
            <label class="header-radio">
                <h3>Special</h3>
                <div class="right">
                    <h5>PHP2500.00</h5>
                    <input type="radio" name="r_type" id="specialRadio" class="special" value="Special" required>
                </div>
            </label>
        </div>
    </div>
            ...
            <br>
            <div class="input-b">
                <label for="per_head" style="color:#fff;">Additional Godparents per head: </label>
            </div>

            <div id="option1" style="display:none;">
        <hr>
        <div class="input-s" style="display:flex; justify-content:space-between; align-center:center; background:skyblue; padding:10px; border-radius:5px; margin-bottom:10px; margin-top:10px;">
            <h5 style="color:#000;">PHP50.00</h5>
            <input type="number" name="per_head" id="regularInput" placeholder="Enter number" style="height:2.5rem; margin:auto; margin-right:40px;">
        </div>
    </div>

    <div id="option2" style="display:none;">
        <hr>
        <div class="input-s" style="display:flex; justify-content:space-between; align-center:center; background:#90EE90; padding:10px; border-radius:5px; margin-bottom:10px; margin-top:10px;">
            <h5 style="color:#000;">PHP100.00</h5>
            <input type="number" name="per_head" id="specialInput" placeholder="Enter number" style="height:2.5rem;  margin:auto; margin-right:40px">
        </div>
    </div>
            ...
            <div class="input-b">
               ...
               <input type="file" id="validId" name="validId[]" accept=".jpg,.jpeg,.png,.pdf,.doc,.docx" multiple required>
           </div>
            <div class="input-b">
            ...
            <div class="input-b">
                ...
                <input type="text" id="amount" name="amount" value="<?php echo isset($amount) ? htmlspecialchars($amount) : '0.00'; ?>" readonly>
            </div>
            <div class="input-b">
    ...
    const specialSlotOptions = [
        "Special 8:00AM - 10:00AM",
        "Special 10:00AM - 12:00PM",
        "Special 1:00PM - 3:00PM",
        "Special 3:00PM - 5:00PM"
    ];

    // Event listener for Regular Radio Button
    regularRadio.addEventListener('change', () => {
        if (regularRadio.checked) {
            option1.style.display = 'block'; // Show option1
            option2.style.display = 'none'; // Hide option2
            regularInput.disabled = false;  // Enable option1 input
            specialInput.disabled = true;   // Disable option2 input
            specialInput.value = '';        // Clear value from option2
            updateAmount(200); // Regular base amount
            setOpacity(regularContainer, specialContainer);
            updateTimeSlotOptions(regularSlotOptions); // Update time slots for Regular
        }
    });

    // Event listener for Special Radio Button
    specialRadio.addEventListener('change', () => {
        if (specialRadio.checked) {
            option1.style.display = 'none'; // Hide option1
            option2.style.display = 'block'; // Show option2
            specialInput.disabled = false;  // Enable option2 input
            regularInput.disabled = true;   // Disable option1 input
            regularInput.value = '';        // Clear value from option1
            updateAmount(2500); // Special base amount
            setOpacity(specialContainer, regularContainer);
            updateTimeSlotOptions(specialSlotOptions); // Update time slots for Special
        }
    });

    // Event listener for Regular Input
    regularInput.addEventListener('input', () => {
        if (regularRadio.checked) {
            const perHead = parseInt(regularInput.value || '0', 10);
            updateAmount(200 + perHead * 50); // 200 base + 50 per head
        }
    });

    // Event listener for Special Input
    specialInput.addEventListener('input', () => {
        if (specialRadio.checked) {
            const perHead = parseInt(specialInput.value || '0', 10);
            updateAmount(2500 + perHead * 100); // 2500 base + 100 per head
        }
    });

    // Function to update the Time Slot Dropdown
    function updateTimeSlotOptions(options) {
    ...
    // Function to set opacity for selected/unselected containers
    function setOpacity(selected, unselected) {
        selected.style.opacity = '1';
        unselected.style.opacity = '0.5';
    }
</script>
