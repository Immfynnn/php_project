...
// Fetch ongoing and rescheduled reservations
$sql = "
    SELECT 
        reservation.s_id, 
    ...
    WHERE reservation.s_status IN ('Ongoing', 'Reschedule')";
    ...
    <div class="head-title">
        <div class="left">
            ...
            <ul class="breadcrumb">
                ...
                <li>
                    <a class="active" href="#">Ongoing Reservation</a>
                </li>
            </ul>
        </div>
    </div>
            ...
            <table>
                ...
                <tbody id="user-table-body">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo "
                                <tr data-service-id='" . htmlspecialchars($row['s_id']) . "' onclick='goToDetails(this)' style='cursor: pointer;'>
                                    ...
                                    <td><span style='color:green; font-weight:bold;'>" . htmlspecialchars($row['s_status']) . "</span></td>
                                    <td>";
                            
                            if ($row['s_status'] == 'Approved') {
                                echo "
                                    <form action='reservation_update_ongoing.php' method='POST'>
                                        <input type='hidden' name='service_id' value='" . htmlspecialchars($row['s_id']) . "' style='cursor:pointer;'>
                                        <button type='submit' id='update-btn'>Update</button>
                                    </form>";
                            } elseif ($row['s_status'] == 'Ongoing') {
                                echo "
                                    <form action='reservation_update_completed.php' method='POST'>
                                        <input type='hidden' name='service_id' value='" . htmlspecialchars($row['s_id']) . "' style='cursor:pointer;'>
                                        <button type='submit' id='update-btn'>Update</button>
                                    </form>";
                            }
                        ...
                        echo "
                            <tr>
                                <td colspan='6' style='text-align:center;'>No ongoing reservations found.</td>
                            </tr>
                        ";
                    }
                    ?>
...
function goToDetails(row) {
    ...
    if (serviceId) {
        window.location.href = `view_reservation_details1.php?service_id=${serviceId}`;
    }
}
</script>
