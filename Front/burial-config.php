...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Handle file uploads for valid ID and requirements
    $target_dir = "uploads/";
    $valid_id_files = [];
    $requirements_files = [];
    ...
    $stmt = $conn->prepare("INSERT INTO reservation (uid, service_type, s_description, set_date, time_slot, s_address, valid_id, s_requirements, amount, payment_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    // Convert file names to JSON string for storing in database
    $valid_id_json = json_encode($valid_id_files);
    $requirements_json = json_encode($requirements_files);

    // Bind parameters to the SQL statement
    $stmt->bind_param("isssssssss", $user_id, $service_type, $s_description, $schedule, $time_slot, $s_address, $valid_id_json, $requirements_json, $amount, $payment_type);
    ...
    if ($stmt->execute()) {
        ...
        header("Location: Preview-Details.php");
        exit();
    } else {
