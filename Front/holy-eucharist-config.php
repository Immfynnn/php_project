...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO reservation (uid, service_type, s_description,s_description1, set_date, time_slot, s_address, amount, payment_type, valid_id, s_status) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");

    $valid_id_json = json_encode($valid_id_files);
    $stmt->bind_param("issssssssss", $user_id, $service_type, $s_description,$s_description1, $set_date, $time_slot, $s_address, $amount, $payment_type, $valid_id_json, $s_status);
    ...
    if ($stmt->execute()) {
        ...
        header("Location: holy-eucharist-preview.php");
        exit();
    } else {
