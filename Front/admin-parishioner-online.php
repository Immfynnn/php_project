...
// Query to fetch only online users
$sql = "SELECT * FROM users WHERE user_status = 'Online'";
$result = $conn->query($sql);

// Count the total number of online parishioners
$totalOnline = $result->num_rows;
    ...
    <div class="head-title">
        <div class="left">
            ...
            <ul class="breadcrumb">
                ...
                <li>
                    <a class="active" href="#">Total Online Parishioner</a>
                </li>
            </ul>
        </div>
    </div>
    ...
    <div class="table-data fade-up" style="outline: rgba(0, 0, 0, 0.1) solid 1px; border-radius:20px;">
        <div class="order">
            <div class="header" style="display:flex; justify-content:space-between; padding:20px;">
                <!-- Display the total count of online parishioners -->
                <h3>Online Parishioner: <b style="color:green;"> <?php echo $totalOnline; ?> </b></h3>
                <div class="divleft">
                    <input type="text" id="search-input" placeholder="Search by username" 
                           style="padding:10px; width:400px; border-radius:5px; border:none; outline:solid 1px rgba(0,0,0,.2);">
            ...
            <table>
                <thead>
                    <tr>
                        <th style='padding-left:30px;'>Profile</th>
                        <th>Username</th>
                        <th>Email</th>
                        <th>Gender</th>
                        <th>Active Status</th>                                
                ...
                <tbody id="user-table-body">
                    ...
                    if ($totalOnline > 0) {
                        while($row = $result->fetch_assoc()) {
                            $userImg = empty($row['userimg']) ? 'css/img/default-profile.png' : $row['userimg'];
                            $statusColor = ($row["user_status"] == 'Online') ? 'green' : 'red';
            
                            echo "<tr style='cursor:pointer;' data-username='" . strtolower($row['username']) . "' onclick=\"window.location.href='admin-parishioner-profile.php?uid=" . $row['uid'] . "'\">";
                            echo "<td style='width:80px; padding-left:30px; padding-right:8px;'><img src='" . $userImg . "' alt='Profile Picture' style='width: 50px; height: 50px; border-radius: 50%;'></td>";
                            echo "<td>" . $row["username"] . "</td>";
                            echo "<td>" . $row["email"] . "</td>";
                            echo "<td>" . $row["gender"] . "</td>";
                            echo "<td style='color: " . $statusColor . ";'>" . $row["user_status"] . "</td>";
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='8'>No online users found</td></tr>";
                    }
                    ?>  
                </tbody>
            </table>
