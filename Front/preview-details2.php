...
$sqlReservation = "SELECT * FROM reservation WHERE s_id = ?";
...
// Fetch service details (e.g., valid_id, s_requirements)
$serviceDetails = [];
$sqlServiceDetails = "SELECT valid_id, s_requirements FROM reservation WHERE s_id = ?";
$stmtServiceDetails = $conn->prepare($sqlServiceDetails);
$stmtServiceDetails->bind_param('i', $s_id);
$stmtServiceDetails->execute();
$serviceDetailsResult = $stmtServiceDetails->get_result();

if ($serviceDetailsResult->num_rows > 0) {
    $serviceDetails = $serviceDetailsResult->fetch_assoc();
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    <div class="b-header">
        <h1>Burial Reservation Details</h1>
    </div>

    <center>
        <div class="div" style="background:#ff8581be; padding:20px; margin-bottom:10px; border-radius:10px;">
            <h3 style="margin-bottom:10px; font-size:25px;"><i class='bx bx-error-circle'></i> Review the Details Before Proceeding</h3>
            <p>Please take a moment to carefully review all the details before moving forward. Ensuring that the information is accurate will help avoid any mistakes or delays.</p>
        </div>
    </center>
    ...
    <form action="reservation-config.php" method="POST">
        ...
        <div class="input-b">
            ...
            <p style="font-size:13px; margin-top:10px;">Inclusion: Mass in House And Mass in Church</p>
        ...
        <div class="input-b">
            ...
            <input type="text" id="time-slot" name="time-slot" value="<?php echo htmlspecialchars($reservation['time_slot']); ?>" readonly>
        </div>

        <!-- Home Address of the Deceased -->
        <div class="input-b">
            <label for="s_address">Home Address of the Deceased:</label>
            <input type="text" id="s_address" name="s_address" value="<?php echo htmlspecialchars($reservation['s_address']); ?>" readonly>
        </div>
        ...
        <div class="input-b">
            ...
            if (!empty($serviceDetails['valid_id'])) {
                // Decode the valid_id JSON array
        ...
        <div class="input-b">
            ...
            <p style="font-size:12px;">Death Certificate</p>
            <?php
        ...
        <!-- Buttons -->
<!-- Buttons -->
<a href="my_reservation-details.php?s_id=<?php echo $s_id; ?>">Done</a>
<a href="my_reservation-update.php?s_id=<?php echo $reservation['s_id']; ?>">Edit Reservation</a>
<?php if (strtolower($reservation['payment_type']) !== 'gcash (scan / send money)' && strtolower($reservation['payment_type']) !== 'over the counter') : ?>
...
<div id="overlay" class="overlay">
        <div class="overlay-content">
            ...
            <p>If you cancel the reservation, the receipt will also automatically delete.</p>
            <button id="cancel-no" class="overlay-button">No</button>
            <button id="cancel-yes" class="overlay-button">Yes</button>
        </div>
    </div>
    ...
    <div class="dialog1">
        ...
        <div class="div-main-prof">
            ...
            <div class="mid-profile">
                <label>
                    <p>ID:</p>
                    <input type="text" value="<?php echo isset($UserDetails['uid']) ? htmlspecialchars($UserDetails['uid']) : 'N/A'; ?>" readonly>
                </label>
                <br>
                <label>
                    <p>Username:</p>
                    <input type="text" value="<?php echo isset($UserDetails['username']) ? htmlspecialchars($UserDetails['username']) : 'N/A'; ?>" readonly>
                </label>
                <br>
                <label>
                    <p>First Name:</p>
                    <input type="text" value="<?php echo isset($UserDetails['firstname']) ? htmlspecialchars($UserDetails['firstname']) : 'N/A'; ?>" readonly>
                </label>
                <br>
                <label>
                    <p>Last Name:</p>
                    <input type="text" value="<?php echo isset($UserDetails['lastname']) ? htmlspecialchars($UserDetails['lastname']) : 'N/A'; ?>" readonly>
                </label>
                <br>
                <label>
                    <p>Gender:</p>
                    <input type="text" value="<?php echo isset($UserDetails['gender']) ? htmlspecialchars($UserDetails['gender']) : 'N/A'; ?>" readonly>
                </label>
            </div>
            <div class="right-profile">
                <label>
                    <p>Age:</p>
                    <input type="text" value="<?php echo isset($UserDetails['age']) ? htmlspecialchars($UserDetails['age']) : 'N/A'; ?>" readonly>
                </label>
                <br>
                <label>
                    <p>Email:</p>
                    <input type="text" value="<?php echo isset($UserDetails['email']) ? htmlspecialchars($UserDetails['email']) : 'N/A'; ?>" readonly>
                </label>
                <br>
                <label>
                    <p>Contact #:</p>
                    <input type="text" value="<?php echo isset($UserDetails['contactnum']) ? htmlspecialchars($UserDetails['contactnum']) : 'N/A'; ?>" readonly>
                </label>
                <br>
                <label>
                    <p>Address:</p>
                    <input type="text" value="<?php echo isset($UserDetails['address']) ? htmlspecialchars($UserDetails['address']) : 'N/A'; ?>" readonly>
                </label>
            </div>
        </div>
